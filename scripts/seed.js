...
async function seedInvoices(client) {
   try {
      ...
      const insertedInvoices = await Promise.all(invoices.map((invoice) => client.sql`
...
async function seedCustomers(client) {
   try {
      ...
      const insertedCustomers = await Promise.all(customers.map((customer) => client.sql`
...
async function seedRevenue(client) {
   try {
      ...
      const insertedRevenue = await Promise.all(revenue.map((rev) => client.sql`
